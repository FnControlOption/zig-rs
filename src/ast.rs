    ...
    pub fn dump(
        ...
        match tag {
            node!(Root) => {
                ...
                writeln!(f, "{indent}{tag:?} [{mode}]")?;
            }
            _ => writeln!(f, "{indent}{tag:?}")?,
        }

        let (lhs_type, rhs_type) = match tag {
            node!(Root) => match self.mode {
                Mode::Zig => {
                    writeln!(f, "{indent}lhs [extra] {lhs}")?;
                    for extra_index in lhs..rhs {
                        let node_index = self.extra_data(extra_index);
                        writeln!(f, "{indent}{extra_index} = [node] {node_index}")?;
                        self.dump(f, self.node(node_index), depth + 1)?;
                    }
                    writeln!(f, "{indent}rhs [extra] {rhs}")?;
                    return Ok(());
                }
                Mode::Zon => {
                    data!(Node, Unused)
        ...
        match lhs_type {
            DataType::Unknown => writeln!(f, "{indent}lhs [?] {lhs}")?,
            DataType::Unused => writeln!(f, "{indent}lhs [unused] {lhs}")?,
            DataType::Node => {
                writeln!(f, "{indent}lhs [node] {lhs}")?;
                if lhs != 0 {
                    self.dump(f, self.node(lhs), depth + 1)?;
                }
            }
        }

        match rhs_type {
            DataType::Unknown => writeln!(f, "{indent}rhs [?] {rhs}")?,
            DataType::Unused => writeln!(f, "{indent}rhs [unused] {rhs}")?,
            DataType::Node => {
                writeln!(f, "{indent}rhs [node] {rhs}")?;
                if rhs != 0 {
                    self.dump(f, self.node(rhs), depth + 1)?;
                }
            }
        }
